...
GLWidget1::GLWidget1 (QWidget *parent , const QGLWidget *shareWidget, Qt::WindowFlags f) : QGLWidget(parent, shareWidget, f)
{
	this->setWindowTitle(QString("Context 1"));
...
void GLWidget1::initializeGL()
{
	...
	Phi = 60*PI/180.0; Tet = 12*PI/180.0; R = 5;
	leftPress = false;
...
void GLWidget1::keyPressEvent(QKeyEvent *event)
{
	//if(event->key() == Qt::Key::Key_Left)
}

void GLWidget1::mousePressEvent(QMouseEvent *event)
{
...
void GLWidget1::mouseReleaseEvent(QMouseEvent *event)
{
	if (event->button() == Qt::MouseButton::LeftButton)
	{
		lastX = event->x();
...
void GLWidget1::mouseMoveEvent(QMouseEvent *event)
{
	if (leftPress)
	{
...
void GLWidget1::wheelEvent(QWheelEvent *event)
{
...
void GLWidget1::timerEvent(QTimerEvent *event)
{
    updateGL();
}
